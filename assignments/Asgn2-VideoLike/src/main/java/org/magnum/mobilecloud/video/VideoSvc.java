...
public class VideoSvc { // implements VideoSvcApi {
	...
	@RequestMapping(value = VideoSvcApi.VIDEO_ID_PATH, method = RequestMethod.GET)
	...
	@RequestMapping(value = VideoSvcApi.VIDEO_LIKE_PATH, method = RequestMethod.POST)
	public void likeVideo(@PathVariable long id, Principal p,
		...
		List<String> usersLiked = v.getUsersLiked();
		if(usersLiked.contains(p.getName())){
			response.sendError(400);
			return;
		}

		usersLiked.add(p.getName());
		v.setUsersLiked(usersLiked);
		v.setLikes(usersLiked.size());
		videos.save(v);
	...
	@RequestMapping(value = VideoSvcApi.VIDEO_UNLIKE_PATH, method = RequestMethod.POST)
	public void unlikeVideo(@PathVariable long id, Principal p,
		...
		List<String> usersLiked = v.getUsersLiked();
		List<String> usersUnliked = v.getUsersUnliked();
		if(!usersLiked.contains(p.getName())){
			response.sendError(400);
			return;
		}
		
		usersLiked.remove(p.getName());
		usersUnliked.add(p.getName());
		v.setUsersLiked(usersLiked);
		v.setUsersUnliked(usersUnliked);
		v.setLikes(usersLiked.size());
		videos.save(v);
		response.setStatus(200);
	}
	...
	@RequestMapping(value = VideoSvcApi.VIDEO_LIKED_BY_PATH, method = RequestMethod.GET)
